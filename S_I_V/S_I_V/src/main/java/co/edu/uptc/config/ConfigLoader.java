...
public class ConfigLoader {
    ...
    static {
        try (InputStream in = ConfigLoader.class.getResourceAsStream("/config.properties")) {
            if (in == null) throw new RuntimeException("config.properties no se encontro");
            props.load(in);
        } catch (IOException e) {
    ...
    public static double getDouble(String key) { return Double.parseDouble(props.getProperty(key)); }
